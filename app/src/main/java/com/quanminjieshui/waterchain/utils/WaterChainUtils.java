 ...
 */
public final class WaterChainUtils {
    ...
    static Context getTopActivityOrApp() {
        if (isAppForeground()) {
            ...
            return topActivity == null ? WaterChainUtils.getApp() : topActivity;
        } else {
            return WaterChainUtils.getApp();
        }
    }

    static boolean isAppForeground() {
        ActivityManager am = (ActivityManager) WaterChainUtils.getApp().getSystemService(Context.ACTIVITY_SERVICE);
        //noinspection ConstantConditions
        List<ActivityManager.RunningAppProcessInfo> info = am.getRunningAppProcesses();
        if (info == null || info.size() == 0) {return false;}
        for (ActivityManager.RunningAppProcessInfo aInfo : info) {
            if (aInfo.importance == ActivityManager.RunningAppProcessInfo.IMPORTANCE_FOREGROUND) {
                return aInfo.processName.equals(WaterChainUtils.getApp().getPackageName());
            }
        }
        return false;
    }
    ...
    static void restoreAdaptScreen() {
        ...
        final DisplayMetrics appDm = WaterChainUtils.getApp().getResources().getDisplayMetrics();
    ...
    public static final class FileProvider4UtilCode extends FileProvider {
        ...
        public boolean onCreate() {
            WaterChainUtils.init(getContext());
            return true;
        }
    }
